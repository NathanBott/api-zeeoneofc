...
var creatorList = ['@NathanBot','@alpha'];
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'NathanBot') return res.json(loghandler.invalidKey)
...
router.get('/tiktod', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'NathanBot') return res.json(loghandler.invalidKey)
